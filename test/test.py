...
def test_3_v2_year(client):
    ...
    good_response = client.get("/api/v2/2019", headers=headers)
    assert good_response.status_code == HTTP_OK
    assert good_response.content_type == "application/json"
    validate(instance=good_response.get_json(), schema=schema)
...
def test_4_v2_open_symbol(client):
    ...
    good_response = client.get("/api/v2/open/AAPL", headers=headers)
    assert good_response.status_code == HTTP_OK
    assert good_response.content_type == "application/json"
    validate(instance=good_response.get_json(), schema=schema)
...
def test_5_v2_close_symbol(client):
    ...
    good_response = client.get("/api/v2/close/AAPL", headers=headers)
    assert good_response.status_code == HTTP_OK
    assert good_response.content_type == "application/json"
    validate(instance=good_response.get_json(), schema=schema)
...
def test_6_v2_high_symbol(client):
    ...
    good_response = client.get("/api/v2/high/AAPL", headers=headers)
    assert good_response.status_code == HTTP_OK
    assert good_response.content_type == "application/json"
    validate(instance=good_response.get_json(), schema=schema)
...
def test_7_v2_low_symbol(client):
    ...
    good_response = client.get("/api/v2/low/AAPL", headers=headers)
    assert good_response.status_code == HTTP_OK
    assert good_response.content_type == "application/json"
    validate(instance=good_response.get_json(), schema=schema)
...
def test_10_routes_without_api_key(client):
    ...
    Verifies that each route returns a HTTP_UNAUTHORIZED Unauthorized status
...
def test_11_v2_year_invalid(client):
    ...
    This test ensures the /api/v2/{YEAR} endpoint returns the appropriate
    status code when an invalid year (e.g., 1980) is provided.
    """
    invalid_year = 1980
...
def test_13_explict_test(client):
    ...
    This test verifies the response against a calculated value to ensure
    the endpoint's accuracy.
    """
