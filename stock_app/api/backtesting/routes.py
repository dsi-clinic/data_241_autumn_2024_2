...
def calc_backtest():
    ...
    start_date_in_stock = execute_stock_q(
        start_date_query, (start_date,), fetch_all=False
    )
    end_date_in_stock = execute_stock_q(
        end_date_query, (end_date,), fetch_all=False
    )
    ...
    back_day = (datetime.strptime(start_date, '%Y-%m-%d') - timedelta(days=back_target)).strftime('%Y-%m-%d')
    ...
    stock_range_df['val_one_day'] = stock_range_df['Date'] - pd.to_timedelta(back_val_one, unit='D')
    stock_range_df['val_two_day'] = stock_range_df['Date'] - pd.to_timedelta(back_val_two, unit='D')

    # Merge to get lagged data for value_1 and value_2
    val_one_df = stock_range_df[['Symbol', 'Date', col_one]].rename(columns={'Date': 'val_one_day', col_one: 'val_one_target'})
    val_two_df = stock_range_df[['Symbol', 'Date', col_two]].rename(columns={'Date': 'val_two_day', col_two: 'val_two_target'})
    merged_df = stock_range_df.merge(val_one_df, on=['Symbol', 'val_one_day'], how='left').merge(val_two_df, on=['Symbol', 'val_two_day'], how='left')
    ...
    for _, row in merged_df.iterrows():
        ...
        if operator == "LT" and row['val_one_target'] < row['val_two_target']:
            day_total = row['Close'] - row['Open'] if purchase_type == "B" else row['Open'] - row['Close']
            total += day_total
            num_observations += 1
        elif operator == "LTE" and row['val_one_target'] <= row['val_two_target']:
            day_total = row['Close'] - row['Open'] if purchase_type == "B" else row['Open'] - row['Close']
            total += day_total
            num_observations += 1

    return jsonify({"return": round(total, 2), 'num_observations': int(num_observations)})
